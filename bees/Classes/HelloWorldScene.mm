...
//	_backGround.position = ccp(winSize.width/2, winSize.height/2);
	...
	[_backGroundNode addChild:_backGround2 z:-2 parallaxRatio:ccp(0.01,0) positionOffset:ccp(_backGround.position.x + _backGround.contentSize.width/2 + _backGround2.contentSize.width/2 - _backGroundNode.position.x * 0.01 -1,
																							winSize.height/2)];

	_backGround3 = [CCSprite spriteWithFile:@"bg3.png"];
	[_backGroundNode addChild:_backGround3 z:-2 parallaxRatio:ccp(0.01,0) positionOffset:ccp(_backGround2.position.x + _backGround2.contentSize.width/2 + _backGround3.contentSize.width/2 - _backGroundNode.position.x * 0.01 - 1,
																							winSize.height/2)];
...
-(void) movePointToNewPosition:(Points*) point{
	...
	point.sprite.position  = ccp(_lastPointLocation.x + screenSize.width, 
								 rnd * screenSize.height/5 );	
...
-(void) movePredatorToNewPosition:(Predator*) predator{
	...
	if (_lastPredatorLocation.x  < _player.position.x + screenSize.width/2){
		_lastPredatorLocation = ccp(_player.position.x + screenSize.width, 0);
	}
	[predator setPos:ccp(_lastPredatorLocation.x + screenSize.width ,rnd * screenSize.height/5 )];	
	
	NSLog(@"predator:%f  %f", predator.position.x, predator.position.y);
	NSLog(@"last predator: %f   %f", _lastPredatorLocation.x, _lastPredatorLocation.y);
...
-(void) loadingTextures{
	...
	_fireBall = nil;
	//[[[Spore alloc] initForNode:_batchNode] retain];
...
-(void) loadingDone{
	...
	_tapToStartSprite = [CCSprite spriteWithFile:@"tapLabel.png"];
...
-(id) init{
	if ((self = [super init])){
		...
		_loadingSprite = [CCSprite spriteWithFile:@"loadingLabel.png"];
		_loadingSprite.position = ccp(_activity.center.x, _activity.center.y + _loadingSprite.contentSize.height);
...
-(void)generatePredators{
	...
	[predator setSpeedMax:1 withRandomRangeOf:0.2f andSteeringForceMax:0.8f withRandomRangeOf:0.15f];
...
-(void) gameOver{
	...
	gameOverLabel.position = ccpAdd(_player.position, ccp(0, screenSize.height/4));
...
-(void) switchPause:(id)sender{
	if (_paused == NO){
		...
		[_loadingScreen runAction:moveDownward];
