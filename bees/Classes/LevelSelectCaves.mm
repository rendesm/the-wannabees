...
@implementation LevelSelectCaves
...
+(id)scene{
	...
	LevelSelectCaves *layer = [LevelSelectCaves node];
...
-(id) init{
	if ((self = [super init])){
		...
		self.background = [CCSprite spriteWithFile:@"levelSelectCaves.png"];
		...
		_buttonHeight = launchButton.contentSize.height/2;
	...
	for (unsigned int i = 0; i < [_easyLevelSprites count]; i++) {
		...
		if (CGRectContainsPoint(sprite.boundingBox, touchLocation)) {            
			if (sprite != self.selectedSprite){
				[[LevelManager sharedManager] setSelectedHillsLevelWithDifficulty:[[ConfigManager sharedManager] difficulty] withNumber:3+i+1];
